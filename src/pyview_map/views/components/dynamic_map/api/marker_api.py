...
def markers_add(
    ...
    APIMarkerSource.push_add(id, name, ll, icon=icon, heading=heading, speed=speed, channel=channel)
...
def markers_update(
    ...
    ll = LatLng.from_list(latLng)
    APIMarkerSource.push_update(id, name, ll, icon=icon, heading=heading, speed=speed, channel=channel)
...
def markers_delete(id: str, channel: str) -> dict:
    APIMarkerSource.push_delete(id, channel=channel)
...
def map_set_view(latLng: list[float], zoom: int, channel: str) -> dict:
    CommandQueue.push(SetViewCmd(latLng=LatLng.from_list(latLng), zoom=zoom), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.panTo")
def map_pan_to(latLng: list[float], channel: str) -> dict:
    CommandQueue.push(PanToCmd(latLng=LatLng.from_list(latLng)), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.flyTo")
def map_fly_to(latLng: list[float], zoom: int, channel: str) -> dict:
    CommandQueue.push(FlyToCmd(latLng=LatLng.from_list(latLng), zoom=zoom), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.fitBounds")
def map_fit_bounds(corner1: list[float], corner2: list[float], channel: str) -> dict:
    CommandQueue.push(FitBoundsCmd(corner1=LatLng.from_list(corner1), corner2=LatLng.from_list(corner2)), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.flyToBounds")
def map_fly_to_bounds(corner1: list[float], corner2: list[float], channel: str) -> dict:
    CommandQueue.push(FlyToBoundsCmd(corner1=LatLng.from_list(corner1), corner2=LatLng.from_list(corner2)), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.setZoom")
def map_set_zoom(zoom: int, channel: str) -> dict:
    CommandQueue.push(SetZoomCmd(zoom=zoom), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.resetView")
def map_reset_view(channel: str) -> dict:
    CommandQueue.push(ResetViewCmd(), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.highlightMarker")
def map_highlight_marker(id: str, channel: str) -> dict:
    CommandQueue.push(HighlightMarkerCmd(id=id), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.highlightPolyline")
def map_highlight_polyline(id: str, channel: str) -> dict:
    CommandQueue.push(HighlightPolylineCmd(id=id), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.followMarker")
def map_follow_marker(id: str, channel: str) -> dict:
    CommandQueue.push(FollowMarkerCmd(id=id), channel=channel)
    return {"ok": True}


@jrpc_service.request("map.unfollowMarker")
def map_unfollow_marker(channel: str) -> dict:
    CommandQueue.push(UnfollowMarkerCmd(), channel=channel)
    return {"ok": True}
...
def polylines_add(
    ...
    APIPolylineSource.push_add(id, name, ll_path, color=color, weight=weight, opacity=opacity, dash_array=dashArray, channel=channel)
...
def polylines_update(
    ...
    ll_path = [LatLng.from_list(p) for p in path]
    APIPolylineSource.push_update(id, name, ll_path, color=color, weight=weight, opacity=opacity, dash_array=dashArray, channel=channel)
...
def polylines_delete(id: str, channel: str) -> dict:
    APIPolylineSource.push_delete(id, channel=channel)
