...
from http_stream_client.jsonrpc.client_sdk import JSONRPCRequest, JSONRPCNotification, JSONRPCResponse, JSONRPCErrorResponse
...
async def main():
    ...
    async with ClientRPC(base_url=BASE_URL, auth_token=AUTH_TOKEN) as rpc:
        ...
        for p in national_parks:
            ...
            list_item = DListItem(id=p["name"], label=p["name"], subtitle=p["description"])
            channel = "places-list" # found by looking at the places_demo file.  The base name of the application and the list component name
